...
class DetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext ctx) {
    return Consumer<ProfileModel>(builder: (_, myProfile, __) => _DetailScreen(profile: profile, myProfile: myProfile));
  }
}
...
class _DetailScreenState extends State<_DetailScreen> {
  ...
  @override
  Widget build(BuildContext ctx) {
    final name = "${widget.profile["first_name"][0].toUpperCase()}${widget.profile["first_name"].substring(1)} ${widget.profile["last_name"][0].toUpperCase()}${widget.profile["last_name"].substring(1)}";
          ...
          body: Center(
            child: Column(
              ...
              children: [
                ...
                if(snapshot.hasData && !snapshot.data) TextButton(
                  onPressed: () {
                    supabase
                      .from("friendships")
                      .insert({"befriender_id": widget.myProfile.id, "befriendee_id": widget.profile["id"]})
                      .then((_) => supabase
                        .from("direct_messages")
                        .insert({"sender_id": widget.myProfile.id, "recipient_id": widget.profile["id"], "message_body": "Hey there! I added you as a friend."})
                      );
                  },
                  child: CText.button(
                    'Add Friend',
